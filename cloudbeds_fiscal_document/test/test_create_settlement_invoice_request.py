# coding: utf-8

"""
    Fiscal document service API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from cloudbeds_fiscal_document.models.create_settlement_invoice_request import CreateSettlementInvoiceRequest

class TestCreateSettlementInvoiceRequest(unittest.TestCase):
    """CreateSettlementInvoiceRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CreateSettlementInvoiceRequest:
        """Test CreateSettlementInvoiceRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CreateSettlementInvoiceRequest`
        """
        model = CreateSettlementInvoiceRequest()
        if include_optional:
            return CreateSettlementInvoiceRequest(
                user_id = 0,
                transaction_ids = [
                    1
                    ],
                folio_ids = [
                    1
                    ],
                exclude_transaction_ids = [
                    1
                    ],
                include_transaction_ids = [
                    1
                    ],
                transaction_id_to_amount = {
                    'key' : 1.337
                    },
                recipient = cloudbeds_fiscal_document.models.recipient_request.RecipientRequest(
                    type = 'GUEST', 
                    id = 1, 
                    tax_document_source = 'GUEST_TAX_ID', ),
                manual_recipient = cloudbeds_fiscal_document.models.manual_recipient_request.ManualRecipientRequest(
                    type = 'PERSON', 
                    name = '', 
                    gender = 'M', 
                    email = '', 
                    birthday = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                    phone = '', 
                    cell_phone = '', 
                    tax_id = '', 
                    address = cloudbeds_fiscal_document.models.manual_recipient_request_address.ManualRecipientRequest_address(
                        country = '', 
                        state = '', 
                        city = '', 
                        address1 = '', 
                        address2 = '', 
                        zip = '', ), 
                    document = cloudbeds_fiscal_document.models.manual_recipient_request_document.ManualRecipientRequest_document(
                        type = 'na', 
                        number = '', 
                        issue_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                        issuing_country = '', 
                        expiration_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), ), ),
                sequence_id = 1,
                due_date_property_timezone = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date()
            )
        else:
            return CreateSettlementInvoiceRequest(
                user_id = 0,
        )
        """

    def testCreateSettlementInvoiceRequest(self):
        """Test CreateSettlementInvoiceRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
